...
interface Props {
  ...
  onSelectDevice: (i: number) => void
  onBack: () => void
  onLock: () => void
}

const PIN_POS = [{cx:195,cy:95},{cx:75,cy:145},{cx:320,cy:60},{cx:300,cy:155}]
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0f1e', display: 'flex', flexDirection: 'column', paddingTop: 59 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
            ...
            <button onClick={refresh} style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(255,255,255,0.05)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#94a3b8' }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ animation: spinning ? 'spin 1s linear infinite' : 'none' }}><path d="M23 4v6h-6M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
            </button>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {/* SVG Map */}
        <div style={{ height: 200, background: '#0d1a2e', position: 'relative', overflow: 'hidden' }}>
          <svg width="100%" height="200" viewBox="0 0 390 200">
            <rect width="390" height="200" fill="#0d1a2e"/>
            <g stroke="rgba(255,255,255,0.03)" strokeWidth="1">
              {[50,100,150].map(y => <line key={y} x1="0" y1={y} x2="390" y2={y}/>)}
              {[78,156,234,312].map(x => <line key={x} x1={x} y1="0" x2={x} y2="200"/>)}
            </g>
            <g fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="3" strokeLinecap="round">
              <path d="M0 80 Q100 70 200 90 Q300 110 390 85"/>
              <path d="M0 130 Q80 140 180 125 Q280 110 390 140"/>
              <path d="M120 0 Q130 100 125 200"/><path d="M260 0 Q255 100 270 200"/>
            </g>
            <g fill="rgba(20,40,70,0.7)">
              <rect x="10" y="20" width="60" height="45" rx="3"/>
              <rect x="140" y="15" width="70" height="55" rx="3"/>
              <rect x="295" y="25" width="80" height="40" rx="3"/>
              <rect x="10" y="110" width="95" height="70" rx="3"/>
              <rect x="170" y="105" width="85" height="55" rx="3"/>
            </g>
            {DEVICES.map((d, i) => {
              const pos = PIN_POS[i]
              const color = d.status === 'online' ? '#2563eb' : d.status === 'alert' ? '#d97706' : '#64748b'
              const isSelected = i === selectedDevice
              return (
                <g key={i} onClick={() => onSelectDevice(i)} style={{ cursor: 'pointer' }}>
                  {isSelected && <circle cx={pos.cx} cy={pos.cy} r="18" fill="none" stroke={color} strokeWidth="1.5" opacity="0.4"/>}
                  <circle cx={pos.cx} cy={pos.cy} r="9" fill={color} stroke="white" strokeWidth="2"/>
                  <text x={pos.cx} y={pos.cy+4} textAnchor="middle" fill="white" fontSize="8" fontWeight="700" fontFamily="sans-serif">{d.model[0]}</text>
                </g>
              )
            })}
          </svg>
        ...
        <div style={{ padding: 16 }}>
          ...
          {DEVICES.map((d, i) => {
            const batColor = d.battery > 50 ? '#4ade80' : d.battery > 20 ? '#fbbf24' : '#f87171'
            const statusColor = d.status === 'online' ? '#4ade80' : d.status === 'alert' ? '#fbbf24' : '#475569'
            return (
              <div key={i} onClick={() => onSelectDevice(i)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 12, borderRadius: 14, cursor: 'pointer', background: i === selectedDevice ? 'rgba(255,255,255,0.05)' : 'transparent', marginBottom: 2, position: 'relative' }}>
                ...
                <div style={{ width: 44, height: 44, borderRadius: 13, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: -2, right: -2, width: 11, height: 11, borderRadius: '50%', background: statusColor, border: '2px solid #0a0f1e' }}/>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  <div style={{ width: 32, height: 5, background: 'rgba(255,255,255,0.08)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 3, background: batColor, width: `${d.battery}%` }}/>
                  </div>
          ...
          {selected && (
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 16, marginTop: 8 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 12 }}>
                {[['Coordinates', selected.coords],['Network', selected.network]].map(([l,v]) => (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {['Play Sound','Lost Mode','Erase'].map(action => (
                  <button key={action} style={{ flex: 1, padding: '10px 6px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', background: action === 'Erase' ? 'rgba(248,113,113,0.05)' : 'rgba(255,255,255,0.04)', color: action === 'Erase' ? '#f87171' : 'white', fontSize: 10, fontWeight: 600, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                    {action}
                  </button>
                ))}
              </div>
