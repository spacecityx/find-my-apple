...
export default function HomeScreen({ active, sessionStart, onLock, onOpenFindMy, onTestBlocked }: Props) {
  const [sessionLabel, setSessionLabel] = useState('Just authenticated')

  useEffect(() => {
    if (!active || !sessionStart) return
    const id = setInterval(() => {
      const secs = Math.floor((Date.now() - sessionStart.getTime()) / 1000)
      setSessionLabel(secs < 60 ? `${secs}s ago` : `${Math.floor(secs/60)}m ago`)
    }, 1000)
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,#0a0f1e,#0d1628)', padding: '68px 20px 40px', display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: 20, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ marginLeft: 'auto', fontSize: 11, color: '#475569' }}>{sessionLabel}</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[['Auth Method','Face ID'],['Security Level','Level 5'],['Encryption','AES-256'],['Devices Found','4 Active']].map(([label, val]) => (
            <div key={label} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 10 }}>
              <div style={{ fontSize: 10, color: '#475569', fontWeight: 500, letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 3 }}>{label}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: label === 'Security Level' ? '#4ade80' : label === 'Devices Found' ? '#60a5fa' : 'white' }}>{val}</div>
            </div>
          ))}
        </div>
      </div>
