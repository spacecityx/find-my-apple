...
export default function CredentialsScreen({ active, onContinue, onSkip, onBack }: Props) {
  ...
  useEffect(() => {
    if (!active) return
    // Load saved credentials from localStorage (Capacitor Preferences would be used in production)
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,#0a0f1e,#0d1628)', display: 'flex', flexDirection: 'column', paddingTop: 59 }}>
      ...
      <div style={{ flex: 1, padding: '0 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <button
          ...
          style={{ width: '100%', padding: '17px 24px', background: email && password ? 'linear-gradient(135deg,#3b82f6,#2563eb)' : 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 17, color: 'white', fontSize: 16, fontWeight: 700, cursor: email && password ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, transition: 'all 0.2s' }}>
